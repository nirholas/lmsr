...
```solidity
import {LMSRHook} from "lmsr/src/hooks/LMSRHook.sol";
...
const hook = getHook("lmsr");
const address = hookAddress("lmsr", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "LMSR"
