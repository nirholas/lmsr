...
/**
 * @title DeployLMSR
 * @notice Deploys LMSR deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
...
contract DeployLMSR is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(LMSRHook).creationCode;
        bytes memory constructorArgs = abi.encode(manager, _b, _swapFeeBps, name_, symbol_);
        ...
        vm.startBroadcast();
        LMSRHook hook = new LMSRHook{salt: salt}(manager, _b, _swapFeeBps, name_, symbol_);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("LMSR")),
