...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(LMSRHook).creationCode, abi.encode(IPoolManager(address(manager)), 50e18, 30, "LMSR LP", "LMSR-LP")
        );
        hook = new LMSRHook{salt: salt}(IPoolManager(address(manager)), 50e18, 30, "LMSR LP", "LMSR-LP");
